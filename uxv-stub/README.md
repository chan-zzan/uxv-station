...
> **B단계를 동기로 갈지 비동기로 갈지는 아직 정하지 않았다.**
> `websockets`에 동기 클라이언트(`websockets.sync.client`)가 있어 `print` → `send` 교체만으로
> 끝날 가능성이 있다. 다만 스텁이 관제기 하트비트를 **받아야** 하게 되면 보내기와 받기를
> 동시에 해야 해서 비동기가 필요해진다. 브리지를 만들 때 정하고 여기에 적는다.
