...
| 원점 | 맵의 로컬 원점 (위경도 아님) |
| 길이 | **미터(m)** |
| 각도 | **라디안(rad)**, 범위 `-π ~ +π` (단 `pitch`만 `-π/2 ~ +π/2`) |
| yaw | 0 = 동쪽(+x), **반시계 양수** |
...
|---|---|---|
| `position` | m | 로컬 ENU |
| `attitude.yaw` | rad | `-π ~ +π` |
| `attitude.pitch` | rad | **`-π/2 ~ +π/2`** — 축별로 범위가 다르다. [ADR 0004](decisions/0004-euler-angles.md) |
| `attitude.roll` | rad | `-π ~ +π` |
| `velocity.linear` | m/s | |
...
| 2026-07-27 | 좌표계 규약 명시 (ENU) | 축·각도 기준이 없어 부호 버그 위험 → [ADR 0002](decisions/0002-coordinate-frame.md) |
| 2026-07-27 | 실제 JSON 형식 확정 | 개념만 있고 직렬화 모양이 없었음 |
| 2026-07-27 | `vehicle_id` 추가 | v2/v3 다중 플랫폼에서 계약이 깨지지 않도록 |
| 2026-07-27 | `SetGimbalTarget.type` → `target_type` | 최상위 `type`과 이름 충돌 |
| 2026-07-27 | 짐벌각 기준을 차체 상대각으로 명시 | 실제 하드웨어와 동일. 안정화 동작이 값 변화로 드러남 |
| 2026-07-27 | `Heartbeat` 메시지 삭제, `VehicleState` 겸용 | 10Hz 스트림 자체가 생존 신호. 중복 제거 |
| 2026-07-27 | 연결 방향·포트·경로 확정 | 브리지만 서버. 타임아웃 1초 |
| 2026-07-28 | `attitude` 범위를 축별로 분리 (`pitch`만 `±π/2`) | 오일러각 표현의 유일성 — `pitch`가 `±π/2`를 넘으면 다른 `(yaw, roll)` 조합과 같은 자세가 된다 → [ADR 0004](decisions/0004-euler-angles.md) |
